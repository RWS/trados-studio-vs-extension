...
#& "$PSScriptRoot\apply-version-to-sourcefiles.ps1" "$PathToVersion\src\Sdl" "$VersionString" "*.info.cs,*.rc,AssemblyInfo.cpp" "AssemblyFileVersion;AssemblyVersionAttribute;FileVersion" "$ProductIdentifier"


param (
    $Path = "$psscriptroot\..\..\..\src\Sdl",
    $VersionNumber ="16.0.0.42",
    $FilenamePattern="*.info.cs,*.rc,AssemblyInfo.cpp",
    $Field="AssemblyFileVersion;AssemblyVersionAttribute;FileVersion",
    $productId="Studio16",
...
$Copyright="Copyright © 2000 - $ThisYear RWS Holdings plc or affiliates."

$CPPCopyright=$Copyright -replace "©" , "\xa9"
$CPPCopyright=$CPPCopyright -replace "`"" , "\x22"
$CPPCopyright=$CPPCopyright -replace "-" , "\x2d"

$cSharpCopyright=$Copyright -replace "`"" , "\`""
...
if($files)
{
    ...
    foreach ($file in $files) {
		...
		else 
		{
			...
			{
                Write-Debug "Updating the '$attr' version"

                if($extension -eq ".cs" -or $extension -eq ".cpp")
                {
                    $newContent = $newContent -replace "$attr\(`"$VersionRegex", "$attr(`"$NewVersion" 
                    $newContent = $newContent -replace "AssemblyCopyright\(`".*`"", "AssemblyCopyright(`"$cSharpCopyright`""
                    $newContent = $newContent -replace "AssemblyConfiguration\(`".*`"", "AssemblyConfiguration(`"$AssemblyConfig`""
                    $newContent = $newContent -replace "AssemblyProduct\(`".*`"", "AssemblyProduct(`"$ProductName`""
                    $newContent = $newContent -replace "AssemblyInformationalVersion\(`".*`"", "AssemblyInformationalVersion(`"$productId`""
                    $newContent = $newContent -replace "AssemblyCopyrightAttribute\(`".*`"", "AssemblyCopyrightAttribute(`"$CPPCopyright`""
